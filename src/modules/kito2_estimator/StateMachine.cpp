...
void StateMachine::StateEngine(const StateMapRowEx* const pStateMapEx)
{
	...
	while (m_eventGenerated)
	{
		...
		const ExitBase* exit_z = pStateMapEx[m_currentState].Exit;
		...
		if (guardResult == TRUE)
		{
			...
			{
				// Execute the state exit_z action on current state before switching to new state
				if (exit_z != NULL)
					exit_z->InvokeExitAction(this);

				// Execute the state entry action on the new state
				if (entry != NULL)
					entry->InvokeEntryAction(this, pDataTemp);

				// Ensure exit_z/entry actions didn't call InternalEvent by accident
